...
const MyDriveTable: React.FC<{ filesArray: any[] }> = (props) => {
  ...
  const addFileToArray = (name: string) => {
  ...
  const handleMouseClick = (event: React.MouseEvent<unknown>, name: string) => {
    ...
    if (event.ctrlKey)
      addFileToArray(name); // click on multiple files with ctrl key
    else setSelected([name]); // click on single file
  };
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      ...
                      onContextMenu={(event) =>
                        handleMouseClick(event, row.name)
                      }
                      onKeyDown={(event) =>
